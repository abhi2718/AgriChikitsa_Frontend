...
class BookmarkFeed extends StatefulHookWidget {
  final feed;
...
class _BookmarkFeedState extends State<BookmarkFeed> {
  ...
  Widget build(BuildContext context) {
    ...
    final isBookMarked = useState(widget.feed['bookmarks'].contains(userInfo.sId));
    ...
    void handleBookMark() {
      useViewModel.toggleTimeline(context, widget.feed['_id'], userInfo.sId!, homeViewModel);
                    ...
                    children: [
                      CustomTextWidget(
                        ...
                        textStyle: const TextStyle(
                            fontSize: 15, fontWeight: FontWeight.w400, color: Colors.black),
                        maxLines:
                            useViewModel.isExpandedBookMarkFeed(widget.feed['_id']) ? null : 2,
                      ),
                      if (widget.feed["hindiCaption"].length > 140)
                        InkWell(
                          onTap: () => useViewModel.toggleExpandBookMarkFeed(widget.feed['_id']),
                          child: useViewModel.isExpandedBookMarkFeed(widget.feed['_id'])
                              ? Container()
                              : const BaseText(
                                  title: "Read More",
                      ...
                      Text(
                        homeViewModel.getTimeAgo(widget.feed['createdAt'], context),
                        style: TextStyle(
