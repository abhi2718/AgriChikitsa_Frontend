...
import 'package:agriChikitsa/screens/tab.screens/agPlus.screen/medicine.screen/widgets/medicine_carousel.dart';
...
class MedicineScreen extends HookWidget {
  const MedicineScreen({super.key, required this.method, this.showCalculator = true});
          ...
          child: ConstrainedBox(
            ...
            child: Padding(
              ...
              child: SingleChildScrollView(
                child: Consumer<MedicineViewModel>(builder: (context, provider, _) {
                          ...
                          onTap: provider.isCalculating
                              ...
                              : () {
                                  if (provider.validateDosageInputs(context)) {
                                    provider.calculateDosage(context, method["_id"]).then((result) {
  ...
  Widget build(BuildContext context) {
    ...
    final useViewModel = useMemoized(() => Provider.of<MedicineViewModel>(context, listen: false));
    useEffect(() {
      useViewModel.reinitializeMedicine();
      WidgetsBinding.instance.addPostFrameCallback((_) {
        useViewModel.getChemicalsList(context, method["_id"]);
      });
      return null;
    }, []);
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(
            method[AppLocalization.of(context).locale.toString() == "en" ? "nameEn" : "nameHi"],
        ...
        body: Consumer<MedicineViewModel>(builder: (context, provider, _) {
          return provider.isChemicalDataLoading
              ? const Center(
              ...
              : SingleChildScrollView(
                  child: Column(
                    children: [
                      SizedBox(
                        height: provider.chemicalsList.isEmpty ? 0 : 32,
                      ),
                      provider.chemicalsList.isEmpty
                          ? SizedBox.shrink()
                          : WeedCarouselWithFeedback(
                              chemicals: provider.chemicalsList,
                              medicineViewModel: provider,
                            ),
                      Container(
                      ...
                      !showCalculator
                          ...
                          : Container(
                              margin:
                                  const EdgeInsets.only(top: 0, left: 14, right: 14, bottom: 22),
                              padding: const EdgeInsets.symmetric(vertical: 22, horizontal: 14),
                              width: double.infinity,
                              decoration: BoxDecoration(
                                  color: AppColor.whiteColor,
